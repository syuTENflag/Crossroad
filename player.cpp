...
int player::model_handle_ojo;		// なんかしろいやつのハンドル
int player::model_handle_asn;	// ゆかりさんのハンドル
int player::model_handle_reimu;	// ぷりんつおいげんさんのハンドル

player::player() {
	// 3Dモデルの読み込み
	model_handle_ojo = MV1LoadModel("./resorces/OJOmodel/dorothy_mmd.pmx");
	model_handle_asn = MV1LoadModel("./resorces/ASNmodel/asn.pmx");
	model_handle_reimu = MV1LoadModel("./resorces/REIMUmodel/霊夢v2.pmx");

	// 初期化処理
	initialize(GAME_CHARA_ASN, 20.0f, VGet(0.0f, 0.0f, 0.0f), false);
}

player::player(int name, VECTOR pos) {
	switch (name) {
	case GAME_CHARA_OJO: initialize(name, 20.0f, pos, true); break;
	case GAME_CHARA_ASN: initialize(name, 14.0f, pos, true); break;
	case GAME_CHARA_REIMU: initialize(name, 14.0f, pos, true); break;
	}
}
...
void player::initialize(int name, float extendf, VECTOR pos, bool anim_load) {
	// 3Dモデルの読み込み(複製)
	switch (name) {
	case GAME_CHARA_OJO: model_handle = MV1DuplicateModel(model_handle_ojo); break;
	case GAME_CHARA_ASN:	model_handle = MV1DuplicateModel(model_handle_asn); break;
	case GAME_CHARA_REIMU:	model_handle = MV1DuplicateModel(model_handle_reimu); break;
	}
	...
	model_position = VGet(320.0f + pos.x, -200.0f + pos.y, 600.0f + pos.z);		// 3Dモデルの座標の格納
